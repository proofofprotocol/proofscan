...
export interface CatalogSource {
  ...
  /** pfscan secret key for API key (if auth required) */
  secretKey?: string;
...
export const CATALOG_SOURCES: CatalogSource[] = [
  ...
  },
  {
    ...
    authRequired: true,
    secretKey: 'catalog.smithery.apiKey',
 ...
 * This will be set by catalog.ts to integrate with pfscan secret
 */
...
/**
 * Get the API key for a source using pfscan secret
 */
...
export function getAuthErrorMessage(source: CatalogSource): string {
  ...
  return `${source.name} requires API key. Set it with: pfscan secret set ${source.secretKey}`;
}
