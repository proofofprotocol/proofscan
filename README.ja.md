...
- 🔧 **[API ドキュメント](docs/API.md)** - 開発者向け TypeScript API (English)
...
MCP ツール操作:
  ...
  registry      ローカルコネクタ自動検出
  runners       パッケージランナー管理（npx, uvx）

A2A エージェント:
  agent         A2A エージェント管理（add, ls, scan）
  task          A2A タスク管理（ls, show, cancel）
...
ショートカット:
  v=view  t=tree  s=scan  st=status  a=archive  c=config  cat=catalog
```
...
- **[API ドキュメント](docs/API.md)** - TypeScript API と EventLine モデル (English)
- **[アーキテクチャ](docs/ARCHITECTURE.md)** - 内部設計とデータベーススキーマ (English)
